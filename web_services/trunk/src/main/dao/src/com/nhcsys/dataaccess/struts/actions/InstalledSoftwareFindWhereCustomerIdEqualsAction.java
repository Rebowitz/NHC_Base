...
public class InstalledSoftwareFindWhereCustomerIdEqualsAction extends NhcBaseAction
{
	/**
	 * Method 'execute'
	 * 
	...
	public ActionForward handle(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		try {
			// parse parameters
			long _customerId = Long.parseLong( request.getParameter("customerId") );
		
			// create the DAO class
			InstalledSoftwareDao dao = InstalledSoftwareDaoFactory.create();
		
			// execute the finder
			InstalledSoftware dto[] = dao.findWhereCustomerIdEquals(_customerId);
