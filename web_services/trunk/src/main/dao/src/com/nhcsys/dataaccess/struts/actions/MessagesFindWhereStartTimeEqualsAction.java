...
public class MessagesFindWhereStartTimeEqualsAction extends NhcBaseAction
{
	/**
	 * Method 'execute'
	 * 
	...
	public ActionForward handle(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		try {
			// parse parameters
			java.util.Date _startTime = (request.getParameter("startTime") == null || request.getParameter("startTime").trim().length()==0) ? null : DateFormat.getDateTimeInstance().parse( request.getParameter("startTime"));
		
			// create the DAO class
			MessagesDao dao = MessagesDaoFactory.create();
		
			// execute the finder
			Messages dto[] = dao.findWhereStartTimeEquals(_startTime);
