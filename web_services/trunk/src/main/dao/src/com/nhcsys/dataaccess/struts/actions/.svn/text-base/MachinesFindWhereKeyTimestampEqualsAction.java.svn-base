...
public class MachinesFindWhereKeyTimestampEqualsAction extends NhcBaseAction
{
	/**
	 * Method 'execute'
	 * 
	...
	public ActionForward handle(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		try {
			// parse parameters
			long _keyTimestamp = Long.parseLong( request.getParameter("keyTimestamp") );
		
			// create the DAO class
			MachinesDao dao = MachinesDaoFactory.create();
		
			// execute the finder
			Machines dto[] = dao.findWhereKeyTimestampEquals(_keyTimestamp);
