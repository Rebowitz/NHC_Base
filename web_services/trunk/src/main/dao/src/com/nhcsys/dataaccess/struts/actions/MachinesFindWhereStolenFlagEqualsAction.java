...
public class MachinesFindWhereStolenFlagEqualsAction extends NhcBaseAction
{
	/**
	 * Method 'execute'
	 * 
	...
	public ActionForward handle(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		try {
			// parse parameters
			int _stolenFlag = Integer.parseInt( request.getParameter("stolenFlag") );
		
			// create the DAO class
			MachinesDao dao = MachinesDaoFactory.create();
		
			// execute the finder
			Machines dto[] = dao.findWhereStolenFlagEquals(_stolenFlag);
