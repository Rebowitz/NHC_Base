...
public class MachinesFindWhereRegisteredFlagEqualsAction extends NhcBaseAction
{
	/**
	 * Method 'execute'
	 * 
	...
	public ActionForward handle(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		try {
			// parse parameters
			int _registeredFlag = Integer.parseInt( request.getParameter("registeredFlag") );
		
			// create the DAO class
			MachinesDao dao = MachinesDaoFactory.create();
		
			// execute the finder
			Machines dto[] = dao.findWhereRegisteredFlagEquals(_registeredFlag);
