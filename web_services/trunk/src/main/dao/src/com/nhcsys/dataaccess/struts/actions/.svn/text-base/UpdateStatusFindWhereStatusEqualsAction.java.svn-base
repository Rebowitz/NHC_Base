...
public class UpdateStatusFindWhereStatusEqualsAction extends NhcBaseAction
{
	/**
	 * Method 'execute'
	 * 
	...
	public ActionForward handle(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		try {
			// parse parameters
			java.lang.String _status = request.getParameter("status");
		
			// create the DAO class
			UpdateStatusDao dao = UpdateStatusDaoFactory.create();
		
			// execute the finder
			UpdateStatus dto[] = dao.findWhereStatusEquals(_status);
