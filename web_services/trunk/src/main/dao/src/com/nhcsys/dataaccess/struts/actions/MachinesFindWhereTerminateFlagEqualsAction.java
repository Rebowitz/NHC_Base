...
public class MachinesFindWhereTerminateFlagEqualsAction extends NhcBaseAction
{
	/**
	 * Method 'execute'
	 * 
	...
	public ActionForward handle(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		try {
			// parse parameters
			int _terminateFlag = Integer.parseInt( request.getParameter("terminateFlag") );
		
			// create the DAO class
			MachinesDao dao = MachinesDaoFactory.create();
		
			// execute the finder
			Machines dto[] = dao.findWhereTerminateFlagEquals(_terminateFlag);
