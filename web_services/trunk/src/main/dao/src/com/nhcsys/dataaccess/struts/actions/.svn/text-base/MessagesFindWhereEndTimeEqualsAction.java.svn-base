...
public class MessagesFindWhereEndTimeEqualsAction extends NhcBaseAction
{
	/**
	 * Method 'execute'
	 * 
	...
	public ActionForward handle(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		try {
			// parse parameters
			java.util.Date _endTime = (request.getParameter("endTime") == null || request.getParameter("endTime").trim().length()==0) ? null : DateFormat.getDateTimeInstance().parse( request.getParameter("endTime"));
		
			// create the DAO class
			MessagesDao dao = MessagesDaoFactory.create();
		
			// execute the finder
			Messages dto[] = dao.findWhereEndTimeEquals(_endTime);
