...
public class ServerRuntimeConfigFindWhereConfigIdEqualsAction extends NhcBaseAction
{
	/**
	 * Method 'execute'
	 * 
	...
	public ActionForward handle(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		try {
			// parse parameters
			long _configId = Long.parseLong( request.getParameter("configId") );
		
			// create the DAO class
			ServerRuntimeConfigDao dao = ServerRuntimeConfigDaoFactory.create();
		
			// execute the finder
			ServerRuntimeConfig dto[] = dao.findWhereConfigIdEquals(_configId);
