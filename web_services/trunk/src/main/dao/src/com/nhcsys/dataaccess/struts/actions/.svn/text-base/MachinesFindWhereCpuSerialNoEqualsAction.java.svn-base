...
public class MachinesFindWhereCpuSerialNoEqualsAction extends NhcBaseAction
{
	/**
	 * Method 'execute'
	 * 
	...
	public ActionForward handle(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		try {
			// parse parameters
			java.lang.String _cpuSerialNo = request.getParameter("cpuSerialNo");
		
			// create the DAO class
			MachinesDao dao = MachinesDaoFactory.create();
		
			// execute the finder
			Machines dto[] = dao.findWhereCpuSerialNoEquals(_cpuSerialNo);
