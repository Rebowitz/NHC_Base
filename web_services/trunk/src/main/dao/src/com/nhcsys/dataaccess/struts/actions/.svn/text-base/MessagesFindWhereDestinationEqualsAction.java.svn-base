...
public class MessagesFindWhereDestinationEqualsAction extends NhcBaseAction
{
	/**
	 * Method 'execute'
	 * 
	...
	public ActionForward handle(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		try {
			// parse parameters
			java.lang.String _destination = request.getParameter("destination");
		
			// create the DAO class
			MessagesDao dao = MessagesDaoFactory.create();
		
			// execute the finder
			Messages dto[] = dao.findWhereDestinationEquals(_destination);
