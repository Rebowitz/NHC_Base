...
public class MessagesFindWherePopupDurationEqualsAction extends NhcBaseAction
{
	/**
	 * Method 'execute'
	 * 
	...
	public ActionForward handle(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		try {
			// parse parameters
			long _popupDuration = Long.parseLong( request.getParameter("popupDuration") );
		
			// create the DAO class
			MessagesDao dao = MessagesDaoFactory.create();
		
			// execute the finder
			Messages dto[] = dao.findWherePopupDurationEquals(_popupDuration);
