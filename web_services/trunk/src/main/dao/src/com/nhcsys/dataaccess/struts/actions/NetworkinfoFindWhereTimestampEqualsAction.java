...
public class NetworkinfoFindWhereTimestampEqualsAction extends NhcBaseAction
{
	/**
	 * Method 'execute'
	 * 
	...
	public ActionForward handle(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		try {
			// parse parameters
			java.util.Date _timestamp = (request.getParameter("timestamp") == null || request.getParameter("timestamp").trim().length()==0) ? null : DateFormat.getDateTimeInstance().parse( request.getParameter("timestamp"));
		
			// create the DAO class
			NetworkinfoDao dao = NetworkinfoDaoFactory.create();
		
			// execute the finder
			Networkinfo dto[] = dao.findWhereTimestampEquals(_timestamp);
